<nav x-data="{ open: false }" class="bg-white border-b border-slate-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <!-- Meta Performance Report Dropdown -->
                    <div class="hidden sm:flex sm:items-center sm:ms-6">
                        <x-dropdown align="right" width="48">
                            <x-slot name="trigger">
                                <button class="inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium leading-5 text-slate-500 hover:text-slate-700 hover:border-slate-300 focus:outline-none focus:text-slate-700 focus:border-slate-300 transition duration-150 ease-in-out">
                                    <div>{{ __('Meta Performance Report') }}</div>
                                    <div class="ms-1">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link :href="route('reports.meta_performance.campaign')">
                                    {{ __('Campaign') }}
                                </x-dropdown-link>
                                <x-dropdown-link :href="route('reports.meta_performance.adset')">
                                    {{ __('Ad Set') }}
                                </x-dropdown-link>
                                <x-dropdown-link :href="route('reports.meta_performance.ad')">
                                    {{ __('Ad') }}
                                </x-dropdown-link>
                            </x-slot>
                        </x-dropdown>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('reports.meta_performance.campaign')" :active="request()->routeIs('reports.meta_performance.campaign')">
                {{ __('Meta Performance (Campaign)') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('reports.meta_performance.adset')" :active="request()->routeIs('reports.meta_performance.adset')">
                {{ __('Meta Performance (Ad Set)') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('reports.meta_performance.ad')" :active="request()->routeIs('reports.meta_performance.ad')">
                {{ __('Meta Performance (Ad)') }}
            </x-responsive-nav-link>
